 ...
 * DB, no Redis. Expected numbers are hand-derived from RuleBasedSimulationEngine's
...
class SimulationEngineTest {
    ...
    /** RuleBasedSimulationEngine's formulas never read sessionId — one fixed value is fine for every session helper below. */
    private val dummySessionId = UUID.randomUUID()

    // ---- coupon (PLAN.md step 4) ----

    @Test
    fun `baseline traffic with no incident is stable`() {
        val state = RuleBasedSimulationEngine.computeState(couponSession(incidentActive = false))
    ...
    fun `the coupon incident overloads both read and write paths with default traits`() {
        val state = RuleBasedSimulationEngine.computeState(couponSession(incidentActive = true))
    ...
    fun `strengthening the rate limit fixes the read axis and partially recovers writes`() {
        ...
        session = RuleBasedSimulationEngine.applyAction(session, SimulationActionType.STRENGTHEN_RATE_LIMIT)
    ...
    fun `increasing cache TTL only affects the read axis, not writes`() {
        ...
        val before = RuleBasedSimulationEngine.computeState(session)

        session = RuleBasedSimulationEngine.applyAction(session, SimulationActionType.INCREASE_CACHE_TTL)
        val after = RuleBasedSimulationEngine.computeState(session)
    ...
    fun `all three coupon actions together fully recover the session to a stable state`() {
        ...
        session = RuleBasedSimulationEngine.applyAction(session, SimulationActionType.STRENGTHEN_RATE_LIMIT)
        session = RuleBasedSimulationEngine.applyAction(session, SimulationActionType.INCREASE_CACHE_TTL)
        session = RuleBasedSimulationEngine.applyAction(session, SimulationActionType.INCREASE_DB_POOL)
    ...
    fun `an out-of-domain action is rejected`() {
        assertThatThrownBy {
            RuleBasedSimulationEngine.applyAction(couponSession(incidentActive = true), SimulationActionType.ADD_CONSUMERS)
        }.isInstanceOf(IllegalStateException::class.java)
    }

    @Test
    fun `utilization bands match docs ARCHITECTURE md section 6`() {
        assertThat(RuleBasedSimulationEngine.latencyMultiplier(0.3)).isEqualTo(1.0)
        assertThat(RuleBasedSimulationEngine.latencyMultiplier(0.7)).isEqualTo(1.5)
        assertThat(RuleBasedSimulationEngine.latencyMultiplier(0.9)).isEqualTo(3.0)
        assertThat(RuleBasedSimulationEngine.latencyMultiplier(0.99)).isEqualTo(5.0)
        assertThat(RuleBasedSimulationEngine.latencyMultiplier(1.5)).isEqualTo(8.0)

        assertThat(RuleBasedSimulationEngine.errorRateFor(0.3)).isCloseTo(0.001, delta)
        assertThat(RuleBasedSimulationEngine.errorRateFor(1.5)).isCloseTo(0.30, delta)
    }
    ...
    fun `baseline notification traffic is stable`() {
        val state = RuleBasedSimulationEngine.computeState(notificationSession(incidentActive = false))
    ...
    fun `the notification incident collapses consumer throughput and backs up the queue`() {
        val state = RuleBasedSimulationEngine.computeState(notificationSession(incidentActive = true))
    ...
    fun `no single notification action alone fully recovers the session`() {
        val breakerOnly = RuleBasedSimulationEngine.computeState(
            RuleBasedSimulationEngine.applyAction(notificationSession(incidentActive = true), SimulationActionType.ENABLE_CIRCUIT_BREAKER)
        )
        val consumersOnly = RuleBasedSimulationEngine.computeState(
            RuleBasedSimulationEngine.applyAction(notificationSession(incidentActive = true), SimulationActionType.ADD_CONSUMERS)
        )
        val backoffOnly = RuleBasedSimulationEngine.computeState(
            RuleBasedSimulationEngine.applyAction(notificationSession(incidentActive = true), SimulationActionType.ADJUST_RETRY_BACKOFF)
        )
    ...
    fun `all three notification actions together recover the session`() {
        ...
        session = RuleBasedSimulationEngine.applyAction(session, SimulationActionType.ENABLE_CIRCUIT_BREAKER)
        session = RuleBasedSimulationEngine.applyAction(session, SimulationActionType.ADD_CONSUMERS)
        session = RuleBasedSimulationEngine.applyAction(session, SimulationActionType.ADJUST_RETRY_BACKOFF)
    ...
    fun `baseline product-browsing traffic is stable`() {
        val state = RuleBasedSimulationEngine.computeState(productBrowsingSession(incidentActive = false))
    ...
    fun `the product-browsing incident craters the cache hit ratio and overloads DB reads`() {
        val state = RuleBasedSimulationEngine.computeState(productBrowsingSession(incidentActive = true))
    ...
    fun `no single product-browsing action alone fully recovers the session`() {
        val splitOnly = RuleBasedSimulationEngine.computeState(
            RuleBasedSimulationEngine.applyAction(productBrowsingSession(incidentActive = true), SimulationActionType.SPLIT_CACHE_POLICY)
        )
        val singleFlightOnly = RuleBasedSimulationEngine.computeState(
            RuleBasedSimulationEngine.applyAction(productBrowsingSession(incidentActive = true), SimulationActionType.ENABLE_SINGLE_FLIGHT)
        )
        val replicaOnly = RuleBasedSimulationEngine.computeState(
            RuleBasedSimulationEngine.applyAction(productBrowsingSession(incidentActive = true), SimulationActionType.ADD_READ_REPLICA)
        )
    ...
    fun `all three product-browsing actions together recover the session`() {
        ...
        session = RuleBasedSimulationEngine.applyAction(session, SimulationActionType.SPLIT_CACHE_POLICY)
        session = RuleBasedSimulationEngine.applyAction(session, SimulationActionType.ENABLE_SINGLE_FLIGHT)
        session = RuleBasedSimulationEngine.applyAction(session, SimulationActionType.ADD_READ_REPLICA)
    ...
    private fun couponSession(incidentActive: Boolean, traits: DesignTraits = DesignTraits()) =
        SimulationSessionState(sessionId = dummySessionId, domain = RuleBasedSimulationEngine.DOMAIN_COUPON, incidentActive = incidentActive, traits = traits)

    private fun notificationSession(incidentActive: Boolean, traits: DesignTraits = DesignTraits()) =
        SimulationSessionState(sessionId = dummySessionId, domain = RuleBasedSimulationEngine.DOMAIN_NOTIFICATION, incidentActive = incidentActive, traits = traits)

    private fun productBrowsingSession(incidentActive: Boolean, traits: DesignTraits = DesignTraits()) =
        SimulationSessionState(sessionId = dummySessionId, domain = RuleBasedSimulationEngine.DOMAIN_PRODUCT_BROWSING, incidentActive = incidentActive, traits = traits)

    // ---- payment (PLAN.md step 18 — PG degrades -> outbox backlog -> bulkhead vs. shared pool) ----

    @Test
    fun `baseline payment traffic is stable`() {
        val state = RuleBasedSimulationEngine.computeState(paymentSession(incidentActive = false))
    ...
    fun `the payment incident backs up the outbox and pollutes the shared connection pool`() {
        val state = RuleBasedSimulationEngine.computeState(paymentSession(incidentActive = true))
    ...
    fun `no single payment action alone fully recovers the session`() {
        val isolateOnly = RuleBasedSimulationEngine.computeState(
            RuleBasedSimulationEngine.applyAction(paymentSession(incidentActive = true), SimulationActionType.ISOLATE_PAYMENT_POOL)
        )
        val workersOnly = RuleBasedSimulationEngine.computeState(
            RuleBasedSimulationEngine.applyAction(paymentSession(incidentActive = true), SimulationActionType.ADD_DISPATCHER_WORKERS)
        )
        val idempotentOnly = RuleBasedSimulationEngine.computeState(
            RuleBasedSimulationEngine.applyAction(paymentSession(incidentActive = true), SimulationActionType.ENABLE_IDEMPOTENT_PG_RETRY)
        )
    ...
    fun `all three payment actions together recover the session`() {
        ...
        session = RuleBasedSimulationEngine.applyAction(session, SimulationActionType.ADD_DISPATCHER_WORKERS)
        session = RuleBasedSimulationEngine.applyAction(session, SimulationActionType.ENABLE_IDEMPOTENT_PG_RETRY)
        session = RuleBasedSimulationEngine.applyAction(session, SimulationActionType.ISOLATE_PAYMENT_POOL)
    ...
    private fun paymentSession(incidentActive: Boolean, traits: DesignTraits = DesignTraits()) =
        SimulationSessionState(sessionId = dummySessionId, domain = RuleBasedSimulationEngine.DOMAIN_PAYMENT, incidentActive = incidentActive, traits = traits)
    ...
    fun `baseline reservation traffic is stable`() {
        val state = RuleBasedSimulationEngine.computeState(reservationSession(incidentActive = false))
    ...
    fun `the reservation incident collapses lock capacity under contention and abandoned holds`() {
        val state = RuleBasedSimulationEngine.computeState(reservationSession(incidentActive = true))
    ...
    fun `no single reservation action alone fully recovers the session`() {
        val fineGrainedOnly = RuleBasedSimulationEngine.computeState(
            RuleBasedSimulationEngine.applyAction(reservationSession(incidentActive = true), SimulationActionType.ENABLE_FINE_GRAINED_LOCKING)
        )
        val shortenTimeoutOnly = RuleBasedSimulationEngine.computeState(
            RuleBasedSimulationEngine.applyAction(reservationSession(incidentActive = true), SimulationActionType.SHORTEN_HOLD_TIMEOUT)
        )
        val atomicOnly = RuleBasedSimulationEngine.computeState(
            RuleBasedSimulationEngine.applyAction(reservationSession(incidentActive = true), SimulationActionType.ENABLE_ATOMIC_INVENTORY_CHECK)
        )
    ...
    fun `all three reservation actions together recover the session`() {
        ...
        session = RuleBasedSimulationEngine.applyAction(session, SimulationActionType.ENABLE_FINE_GRAINED_LOCKING)
        session = RuleBasedSimulationEngine.applyAction(session, SimulationActionType.SHORTEN_HOLD_TIMEOUT)
        session = RuleBasedSimulationEngine.applyAction(session, SimulationActionType.ENABLE_ATOMIC_INVENTORY_CHECK)
    ...
    private fun reservationSession(incidentActive: Boolean, traits: DesignTraits = DesignTraits()) =
        SimulationSessionState(sessionId = dummySessionId, domain = RuleBasedSimulationEngine.DOMAIN_RESERVATION, incidentActive = incidentActive, traits = traits)
    ...
    fun `baseline batch-settlement run is stable`() {
        val state = RuleBasedSimulationEngine.computeState(batchSettlementSession(incidentActive = false))
    ...
    fun `the batch-settlement incident forces a full restart and duplicate reconciliation without mitigation`() {
        val state = RuleBasedSimulationEngine.computeState(batchSettlementSession(incidentActive = true))
    ...
    fun `no single batch-settlement action alone fully recovers the session`() {
        val checkpointOnly = RuleBasedSimulationEngine.computeState(
            RuleBasedSimulationEngine.applyAction(batchSettlementSession(incidentActive = true), SimulationActionType.ENABLE_CHECKPOINT_RESTART)
        )
        val reduceChunkOnly = RuleBasedSimulationEngine.computeState(
            RuleBasedSimulationEngine.applyAction(batchSettlementSession(incidentActive = true), SimulationActionType.REDUCE_CHUNK_SIZE)
        )
        val idempotentOnly = RuleBasedSimulationEngine.computeState(
            RuleBasedSimulationEngine.applyAction(batchSettlementSession(incidentActive = true), SimulationActionType.ENABLE_IDEMPOTENT_RECONCILIATION)
        )
    ...
    fun `all three batch-settlement actions together recover the session`() {
        ...
        session = RuleBasedSimulationEngine.applyAction(session, SimulationActionType.ENABLE_CHECKPOINT_RESTART)
        session = RuleBasedSimulationEngine.applyAction(session, SimulationActionType.REDUCE_CHUNK_SIZE)
        session = RuleBasedSimulationEngine.applyAction(session, SimulationActionType.ENABLE_IDEMPOTENT_RECONCILIATION)
