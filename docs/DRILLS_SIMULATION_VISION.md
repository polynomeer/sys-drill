...
| Drill Map(의존성 그래프) | **2026-09-14 갱신(2차)**: 공식 7개 시나리오의 `difficulty`가 전부 `MEDIUM`으로 동일해 "진행 경로 데이터 자체가 없다"던 갭 중 난이도 쪽을 V43 마이그레이션으로 먼저 메움(coupon=EASY / notification·product-browsing=MEDIUM / payment·reservation·batch-settlement·autoscaling=HARD). 그 위에 "L1→L2→L3 prerequisite 그래프"의 가장 작은 형태 — EASY→MEDIUM→HARD 3단계 전순서(total order) — 로 advisory 힌트를 추가: `GET /sessions`가 세션의 `difficulty`를 내려주고(`SessionController.resolveScenarioMeta`), 대시보드가 사용자가 완료한 세션의 난이도 집합과 비교해 아직 낮은 티어를 완료하지 않은 MEDIUM/HARD 시나리오에 "선행 추천: 쉬움 난이도 먼저" 문구를 덧붙임(ADR-0030과 같은 철학 — 세션 시작은 절대 안 막음). 특정 시나리오 쌍 간의 임의 의존성(예: "예약은 쿠폰을 선행해야 함")이나 노드/엣지 시각화는 여전히 없음 — 근거 없는 콘텐츠 결정이라 의도적으로 회피 | **부분(3단계 전순서 기반 advisory 힌트까지 확보)**, 임의 페어와이즈 그래프/시각화는 여전히 전무 |
...
| 4 | Drill Map(의존성 그래프) — **보류(2026-09-11), 난이도 슬라이스 완료(2026-09-14)**, **3단계 전순서 advisory 힌트 슬라이스 완료(2026-09-14)**. 공식 7개 시나리오의 실제 난이도(V43 마이그레이션) 위에, `GET /sessions`의 `difficulty` + 대시보드의 `TIER_ORDER`/`completedTiers`로 "아직 쉬운 난이도를 완료하지 않았다"는 것만 안내(세션 시작은 안 막음). 임의 페어와이즈 의존성이나 노드/엣지 그래프 UI는 여전히 보류 | Marketplace 확장 | 평면 목록보다 그래프 탐색이 실제로 더 쓰이는가? |
