...
| Drill Map(의존성 그래프) | **2026-09-14 갱신**: 공식 7개 시나리오의 `difficulty`가 전부 `MEDIUM`으로 동일해 "진행 경로 데이터 자체가 없다"던 갭 중 난이도 쪽을 V43 마이그레이션으로 메움 — 각 도메인의 실제 메커닉(RuleEvaluator 체크 개수, real-infra 지원 여부, 고정 vs 3-variant 적응형 FOLLOWUP)을 근거로 coupon=EASY / notification·product-browsing=MEDIUM / payment·reservation·batch-settlement·autoscaling=HARD로 실제 분화. 기존 대시보드(`dashboard/page.tsx`)가 이미 `scenario.difficulty`를 렌더링하고 있어 프론트 코드 변경 없이 즉시 반영됨. prerequisite/의존성 그래프 필드는 여전히 전무 — 그래프 UI를 만들 실제 엣지 데이터는 아직 없음 | **부분(난이도만 확보)**, 그래프/prerequisite는 여전히 전무 |
...
| 4 | Drill Map(의존성 그래프) — **보류(2026-09-11), 난이도 데이터 슬라이스만 완료(2026-09-14)**. 공식 7개 시나리오의 실제 난이도(coupon=EASY / notification·product-browsing=MEDIUM / payment·reservation·batch-settlement·autoscaling=HARD, V43 마이그레이션)는 부여했으나, prerequisite/의존성 그래프 필드는 여전히 전무 — 그래프 UI는 여전히 보류 | Marketplace 확장 | 평면 목록보다 그래프 탐색이 실제로 더 쓰이는가? |
