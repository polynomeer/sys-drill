...
| [0028](0028-google-login-trusts-userinfo-endpoint-redis-state-and-unknown-placeholder-password.md) | Google login trusts the userinfo endpoint instead of verifying the ID token locally, uses a Redis one-time token for CSRF state, and gives Google-only accounts a password nobody knows |
| [0029](0029-audit-log-scoped-to-organization-actions-recorded-synchronously.md) | The audit log is scoped to organization admin/membership actions, not a platform-wide trail, and is written synchronously in the same transaction as the action it records |
| [0030](0030-onboarding-curriculum-is-advisory-full-replace-and-counts-retroactive-completion.md) | The onboarding curriculum only orders and displays progress — it never gates session start, is edited as a full-replace `PUT`, and counts sessions completed before the curriculum existed |
| [0031](0031-marketplace-scenarios-join-the-public-pool-with-no-payment-in-v1.md) | Marketplace scenarios join the same public pool as Flyway-seeded content instead of a separate namespace, and v1 ships with no payment or approval queue |
| [0032](0032-certification-is-live-computed-and-scoped-to-official-domains.md) | "SysDrill Certified Incident Responder" is a live-computed judgement, never issued or persisted, and only counts official Flyway-seeded domains |
| [0033](0033-assessment-reuses-invitation-shape-but-preview-stays-public-and-status-is-derived.md) | Recruiting assessments reuse the organization invitation's email-bound token shape, but the preview endpoint stays public (unlike invitation preview) and status is derived, never persisted |
| [0034](0034-architecture-linter-v1-is-openapi-only-no-retention-and-private-by-default.md) | Architecture Linter v1 accepts only an OpenAPI spec (not a repository), never persists the raw input, and generated scenarios are private to their creator by a new `Scenario.visibility` axis |
| [0035](0035-diagrams-are-mermaid-text-embedded-in-the-existing-answer-not-a-new-field-or-editor.md) | Architecture diagrams are Mermaid text embedded in the existing free-text answer (not a new field or a drawing editor), reconciling PRD.md's MVP-era editor exclusion with ARCHITECTURE.md's React Flow candidate |
